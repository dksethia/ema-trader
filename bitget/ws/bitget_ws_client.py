...
class BitgetWsClient:
    ...
    def __on_message(self, ws, message):
        ...
        if listenner:
            # EDIT START
            new_df = listenner(message)

            if self.df is None:
                self.df = new_df
                return

            if self.df.index[-1] == new_df.index[-1]:
                # If message is for a candle already in the dataframe, then update the row
                self.df.iloc[-1] = new_df.iloc[-1]
            else:
                # Else If message is for a new candle, then add to the dataframe
